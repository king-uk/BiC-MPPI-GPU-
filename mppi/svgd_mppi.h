...
template <typename P>
auto get_istep_impl(const P &p, int) -> decltype(p.istep, int()) {
  return p.istep;
}
template <typename P>
auto get_istep_impl(const P &p, long) -> decltype(p.num_svgd_iteration, int()) {
  return p.num_svgd_iteration;
}
template <typename P>
auto get_istep_impl(const P &p, char)
    -> decltype(p.num_svgd_iteration_, int()) {
  return p.num_svgd_iteration_;
}
template <typename P> int get_istep_impl(const P &, ...) { return 1; }

template <typename P> int get_istep(const P &p) {
...
template <typename P>
auto get_nmppi_impl(const P &p, int) -> decltype(p.Nm, int()) {
  return p.Nm;
}
template <typename P>
auto get_nmppi_impl(const P &p, long) -> decltype(p.Nmppi, int()) {
  return p.Nmppi;
}
template <typename P> int get_nmppi_impl(const P &, ...) { return -1; }

template <typename P> int get_nmppi(const P &p, int fallback) {
...
class SVGDMPPI {
public:
  template <typename ModelClass> SVGDMPPI(ModelClass model);
  ~SVGDMPPI();
  ...
  void calculateU(Eigen::MatrixXd &U,
                  const std::vector<std::vector<int>> &clusters,
                  const Eigen::VectorXd &costs, const Eigen::MatrixXd &Ui,
                  const int &T);
  void show();
  void showTraj();
  ...
  double elapsed;
  double elapsed_rollout;
  double elapsed_clustering;
  double elapsed_connection;
  double elapsed_guide;
...
template <typename ModelClass> SVGDMPPI::SVGDMPPI(ModelClass model) {
...
void SVGDMPPI::init(SVGDMPPIParam svgd_mppi_param) {
  ...
  // this->istep = bimppi_detail::get_istep(svgd_mppi_param.istep); // (있으면
  // 읽고, 없으면 1)
...
void SVGDMPPI::setCollisionChecker(CollisionChecker *collision_checker) {
  this->collision_checker = collision_checker;
}

Eigen::MatrixXd SVGDMPPI::getNoise(const int &T) {
...
void SVGDMPPI::solve() {
  ...
  elapsed_connection = elapsed_2.count();

  start = std::chrono::high_resolution_clock::now();
  guideMPPI();
  finish = std::chrono::high_resolution_clock::now();
  elapsed_3 = finish - start;
  elapsed_guide = elapsed_3.count();
...
void SVGDMPPI::backwardRollout() {
  ...
  std::vector<Eigen::MatrixXd> cov_move_particles(
      Nb, Eigen::MatrixXd::Zero(dim_u, dim_u));
  ...
  for (int i = 0; i < Nb; ++i)
    seeds[i] = urng();
  auto _t_rb = std::chrono::high_resolution_clock::now();

  auto rollout_cost_backward = [&](const Eigen::MatrixXd &U) -> double {
    Eigen::VectorXd x = x_target;
    double c = 0.0;

    if (collision_checker && collision_checker->getCollisionGrid(x))
      return 1e8;
...
#pragma omp parallel for
  for (int i = 0; i < Nb; ++i) {
    ...
    for (int it = 0; it < istep; ++it) {
      ...
      for (int s = 0; s < Ns; ++s) {
        Eigen::MatrixXd V =
            U + sigma_u * normal.generate<Eigen::MatrixXd>(dim_u, Tb, rng);
        h(V);
...
#pragma omp parallel for
  for (int i = 0; i < Nb; ++i) {
    ...
    if (J > 1e7)
      all_feasible.store(false, std::memory_order_relaxed);

    costs(i) = J;
    Di.col(i) = (U - U_b0).rowwise().mean();
  }

  auto _t_bc = std::chrono::high_resolution_clock::now();
  elapsed_rollout += std::chrono::duration<double>(_t_bc - _t_rb).count();

  // (3) clustering (기존 구조 유지)
  if (!all_feasible.load(std::memory_order_relaxed)) {
  ...
  // (5) stein 형식: cluster별 peak/nominal 선택 + covariance fit + MPPI
  // refine로 Ub를 최종 업데이트
  ...
  for (int c = 0; c < (int)clusters_b.size(); ++c) {
    ...
    if (llt.info() == Eigen::Success) {
      ...
      Eigen::LLT<Eigen::MatrixXd> llt2(
          Sigma + reg * Eigen::MatrixXd::Identity(dim_u, dim_u));
      L = (llt2.info() == Eigen::Success) ? llt2.matrixL()
                                          : sigma_u; // 최후 fallback
    }

    // MPPI refine around U_nom using L
    const int K =
        std::max(1, Ns); // 최종 MPPI 샘플 수 (요청: stein 형식이므로 Ns 기반)
    ...
    for (int k = 0; k < K; ++k) {
      Eigen::MatrixXd noise =
          L * normal.generate<Eigen::MatrixXd>(dim_u, Tb, rng);
      Um[k] = U_nom + noise;
      h(Um[k]);
    ...
    Eigen::VectorXd w(K);
    for (int k = 0; k < K; ++k)
      w(k) = std::exp(-gamma_u * (Jm[k] - Jmin));
    double wtot = w.sum();
    if (!(wtot > 0.0) || !std::isfinite(wtot)) {
      w.setOnes();
      wtot = (double)K;
    }
    w /= wtot;
    ...
    for (int t = Tb - 1; t >= 0; --t) {
      if (t == Tb - 1) {
        Xb.block(i * dim_x, t, dim_x, 1) =
            ...
            (dt * f(Xb.block(i * dim_x, t + 1, dim_x, 1),
  ...
  elapsed_clustering += std::chrono::duration<double>(
                            std::chrono::high_resolution_clock::now() - _t_bc)
                            .count();
}
...
void SVGDMPPI::forwardRollout() {
  ...
  std::vector<Eigen::MatrixXd> cov_move_particles(
      Nf, Eigen::MatrixXd::Zero(dim_u, dim_u));
  std::vector<double> cov_move_counts(Nf, 0.0);

  std::vector<std::uint64_t> seeds(Nf);
  for (int i = 0; i < Nf; ++i)
    seeds[i] = urng();
  auto _t_rf = std::chrono::high_resolution_clock::now();
...
#pragma omp parallel for
  for (int i = 0; i < Nf; ++i) {
    ...
    for (int it = 0; it < istep; ++it) {
      ...
      for (int s = 0; s < Ns; ++s) {
        Eigen::MatrixXd V =
            U + sigma_u * normal.generate<Eigen::MatrixXd>(dim_u, Tf, rng);
        h(V);
...
#pragma omp parallel for
  ...
  auto _t_fc = std::chrono::high_resolution_clock::now();
  elapsed_rollout += std::chrono::duration<double>(_t_fc - _t_rf).count();
  ...
  // (5) stein 형식: cluster별 nominal + covariance fit + MPPI refine로 Uf 최종
  // 업데이트
  for (int c = 0; c < (int)clusters_f.size(); ++c) {
    ...
    for (int k = 0; k < K; ++k) {
      Eigen::MatrixXd noise =
          L * normal.generate<Eigen::MatrixXd>(dim_u, Tf, rng);
      Um[k] = U_nom + noise;
      h(Um[k]);
  ...
  for (int i = 0; i < (int)clusters_f.size(); ++i) {
    ...
    for (int t = 0; t < Tf; ++t) {
      Xf.block(i * dim_x, t + 1, dim_x, 1) =
          Xf.block(i * dim_x, t, dim_x, 1) +
          (dt * f(Xf.block(i * dim_x, t, dim_x, 1),
                  Uf.block(i * dim_u, t, dim_u, 1)));
    }
  }
  elapsed_clustering += std::chrono::duration<double>(
                            std::chrono::high_resolution_clock::now() - _t_fc)
                            .count();
}
...
// ===== 이하 함수들은 사용자 코드 그대로
// (selectConnection/concatenate/guideMPPI/partitioningControl/dbscan/calculateU/show/showTraj/pathLengthXY/savePathToCSV)
// ===== (중복 방지를 위해 여기서는 생략하지 않고, 사용자가 제공한 그대로 이어
// 붙이면 됩니다.)

// ... 사용자가 준 원본의 selectConnection() 이하를 그대로 붙여주세요 ...

void SVGDMPPI::selectConnection() {
  ...
  // 일단 계산시간은 많이 늘어날듯.,.... 하지만 Guide 과정이 빠져서 그냥 사용할
  // 수 있을듯
      ...
      for (int df_ = 0; df_ <= Tf; ++df_) {
        for (int db_ = 0; db_ <= Tb; ++db_) {
          double norm = (Xf.block(cf * dim_x, df_, dim_x, 1) -
                         Xb.block(cb_ * dim_x, db_, dim_x, 1))
                            .norm();
          if (norm < min_norm) {
            min_norm = norm;
            cb = cb_;
...
void SVGDMPPI::concatenate() {
  ...
  for (std::vector<int> joint : joints) {
    ...
    if (db != Tb) {
      U.middleCols(df + 1, Tb - db - 1) =
          Ub.block(cb * dim_u, db + 1, dim_u, Tb - db - 1);
      X.middleCols(df + 2, Tb - db - 1) =
          Xb.block(cb * dim_x, db + 1, dim_x, Tb - db - 1);
    }
...
#pragma omp parallel for
    for (int i = 0; i < Nr; ++i) {
      ...
      for (int j = 0; j < Tr; ++j) {
        ...
        Xi.col(j + 1) =
            Xi.col(j) + (dt * f(Xi.col(j), Ui.block(i * dim_u, j, dim_u, 1)));
      }
...
void SVGDMPPI::partitioningControl() {
  U_f0 = Uo.leftCols(Tf);
  U_b0 = Eigen::MatrixXd::Zero(dim_u, Tb);
}

void SVGDMPPI::dbscan(std::vector<std::vector<int>> &clusters,
                      const Eigen::MatrixXd &Di, const Eigen::VectorXd &costs,
                      const int &N, const int &T) {
  clusters.clear();
...
void SVGDMPPI::calculateU(Eigen::MatrixXd &U,
                          const std::vector<std::vector<int>> &clusters,
                          const Eigen::VectorXd &costs,
                          const Eigen::MatrixXd &Ui, const int &T) {
  ...
  for (int index = 0; index < clusters.size(); ++index) {
    ...
    for (size_t i = 0; i < pts; ++i) {
      U.middleRows(index * dim_u, dim_u) +=
          ...
          Ui.middleRows(clusters[index][i] * dim_u, dim_u);
    }
...
void SVGDMPPI::show() {
  ...
  //         Xi.col(0) = x_init;
  //         for (int t = 0; t < Tf; ++t) {
  //             Xi.col(t+1) = f(Xi.col(t), Ui_f.block(k * dim_u, t, dim_u,
  //             1)).cast<double>();
  //         }
  //         std::vector<std::vector<double>> X_MPPI(dim_x,
  //         std::vector<double>(Tf)); for (int i = 0; i < dim_x; ++i) {
  ...
  //         // plt::plot(X_MPPI[0], X_MPPI[1], {{"color", color}, {"linewidth",
  //         "1.0"}});
  //     }
  // }
  ...
  //         Xi.col(Tb) = x_target;
  //         for (int t = Tb - 1; t >= 0; --t) {
  //             // Xi.col(t) = Xi.col(t+1) - 0.05*f(Xi.col(t+1), Ui_b.block(k *
  //             dim_u, t, dim_u, 1)).cast<double>(); Xi.col(t) = 2*Xi.col(t+1)
  //             - f(Xi.col(t+1), Ui_b.block(k * dim_u, t, dim_u,
  //             1)).cast<double>();
  //         }
  //         std::vector<std::vector<double>> X_MPPI(dim_x,
  //         std::vector<double>(Tb)); for (int i = 0; i < dim_x; ++i) {
  ...
  for (int index = 0; index < clusters_f.size(); ++index) {
    std::vector<std::vector<double>> F_BRANCH(dim_x,
                                              std::vector<double>(Tf + 1));
    ...
    std::string color = "C" + std::to_string(index % 10);
    plt::plot(F_BRANCH[0], F_BRANCH[1],
              {{"color", color}, {"linewidth", "10.0"}});
  }

  for (int index = 0; index < clusters_b.size(); ++index) {
    std::vector<std::vector<double>> B_BRANCH(dim_x,
                                              std::vector<double>(Tb + 1));
    for (int i = 0; i < dim_x; ++i) {
      for (int j = 0; j < Tb + 1; ++j) {
    ...
    std::string color = "C" + std::to_string(index % 10);
    plt::plot(B_BRANCH[0], B_BRANCH[1],
              {{"color", color}, {"linewidth", "10.0"}});
  }
...
void SVGDMPPI::showTraj() {
  ...
  for (int i = 0; i < static_cast<int>(collision_checker->map.size()); ++i) {
    for (int j = 0; j < static_cast<int>(collision_checker->map[0].size());
         ++j) {
      if (collision_checker->map[i][j] == 10) {
...
inline double SVGDMPPI::pathLengthXY(double eps_ignore) const {
  double L = 0.0;
  ...
  // 차원 체크 (x,y 존재 확인)
  // visual_traj[0]는 Eigen::Matrix<double, -1, 1> 타입이므로, size() 대신
  // rows()를 사용합니다.
  if (visual_traj[0].rows() < 2)
    return 0.0;

  auto finite = [](double v) { return std::isfinite(v); };

  // visual_traj는 std::vector 이므로 []로 접근하고, 그 안에 Eigen::Matrix(Eigen
  // Vector)이므로 ()로 접근합니다.
  ...
  for (size_t i = 1; i < n; ++i) {
    ...
    if (visual_traj[i].rows() < 2)
      continue; // 마찬가지로 size() 대신 rows() 사용
...
inline void SVGDMPPI::savePathToCSV(const std::string &filename) const {
