...
// Made public for GPU solver access
public:
  bool is_3d;
  int max_hei;
  bool with_map;
...
inline void CollisionChecker::loadMap(const std::string &file_path,
                               double resolution) {
  map.clear();
  ...
  max_row = map.size();
  max_col = map.empty() ? 0 : map[0].size();
}

inline void CollisionChecker::set3D(double max_hei) {
  this->is_3d = true;
  this->max_hei = max_hei / resolution;
}

inline void CollisionChecker::addCircle(double x, double y, double r) {
...
inline bool CollisionChecker::getCollisionCircle(const Eigen::VectorXd &x) {
  if (with_map) {
...
inline bool CollisionChecker::getCollisionGrid_polygon(const Eigen::VectorXd &x) {
  double dx;
  double dy;
  double distance_2;
  // Circle
  ...
  for (int i = 0; i < rectangles.size(); ++i) {
    if (x(0) < rectangles[i][0]) {
      continue;
    } else if (rectangles[i][1] < x(0)) {
      continue;
    } else if (x(1) < rectangles[i][2]) {
      continue;
    } else if (rectangles[i][3] < x(1)) {
      continue;
    } else {
      return true;
    }
...
inline bool CollisionChecker::getCollisionCircle_polygon(const Eigen::VectorXd &z) {
  Eigen::VectorXd zj;
  double dx;
  double dy;
  double distance_2;
  ...
  // Rectangle
  for (int i = 0; i < rectangles.size(); ++i) {
    if ((z(0) + z(2)) < rectangles[i][0]) {
      continue;
    } else if (rectangles[i][1] < (z(0) - z(2))) {
      continue;
    } else if ((z(1) + z(2)) < rectangles[i][2]) {
      continue;
    } else if (rectangles[i][3] < (z(1) - z(2))) {
      continue;
    } else {
      return true;
    }
...
inline bool CollisionChecker::getCollisionGrid_map(const Eigen::VectorXd &x) {
...
inline bool CollisionChecker::getCollisionCircle_map(const Eigen::VectorXd &z) {
  // Need to check comparison double
