...
public:
  template <typename ModelClass> ClusterMPPI(ModelClass model);
  ~ClusterMPPI();
...
#pragma omp parallel for
    ...
    auto _t_cluster_start = std::chrono::high_resolution_clock::now();
    elapsed_rollout =
        std::chrono::duration<double>(_t_cluster_start - start).count();

    std::vector<std::vector<int>> clusters;

    if (!all_feasible) {
      dbscan(clusters, Di, costs, N, T);
    } else {
      clusters.clear();
    }
    ...
    for (int i = 0; i < clusters.size(); ++i) {
      ...
      for (int j = 0; j < T; ++j) {
        cost += p(Xi.col(j), x_target);
        Xi.col(j + 1) =
            Xi.col(j) + (dt * f(Xi.col(j), U.block(i * dim_u, j, dim_u, 1)));
      }
      cost += p(Xi.col(T), x_target);
      for (int j = 1; j < T + 1; ++j) {
        if (collision_checker->getCollisionGrid(Xi.col(j))) {
    ...
    elapsed_clustering =
        std::chrono::duration<double>(finish - _t_cluster_start).count();
...
void ClusterMPPI::dbscan(std::vector<std::vector<int>> &clusters,
                         const Eigen::MatrixXd &Di,
                         const Eigen::VectorXd &costs, const int &N,
                         const int &T) {
  clusters.clear();
...
void ClusterMPPI::calculateU(Eigen::MatrixXd &U,
                             const std::vector<std::vector<int>> &clusters,
