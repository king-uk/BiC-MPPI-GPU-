    ...
    for (int cb_ = 0; cb_ < clusters_b.size(); ++cb_) {
      for (int df_ = 0; df_ <= Tf; ++df_) {
        for (int db_ = 0; db_ <= Tb; ++db_) {
          ...
          double weight_xy = 0.2;
          double weight_theta = 1.0;
          Eigen::VectorXd xf = Xf.block(cf * dim_x, df_, dim_x, 1);
          Eigen::VectorXd xb = Xb.block(cb_ * dim_x, db_, dim_x, 1);

          double dtheta =
              std::atan2(std::sin(xf(2) - xb(2)), std::cos(xf(2) - xb(2)));

          double dx = xf(0) - xb(0);
          double dy = xf(1) - xb(1);

          double norm = std::sqrt(weight_xy * (dx * dx + dy * dy) +
                                  weight_theta * (dtheta * dtheta));
