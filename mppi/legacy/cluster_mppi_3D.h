...
class ClusterMPPI3D : public MPPI3D {
public:
    template<typename ModelClass>
    ClusterMPPI3D(ModelClass model);
    ~ClusterMPPI3D();
    ...
    void solve() override {
        ...
        for (int i = 0; i < clusters.size(); ++i) {
            ...
            for (int j = 0; j < T; ++j) {
                cost += p(Xi.col(j), x_target);
                Xi.col(j+1) = rk4Step(Xi.col(j), U.block(i * dim_u, j, dim_u, 1));
            }
            cost += p(Xi.col(T), x_target);
            for (int j = 1; j < T+1; ++j) {
                if (checkCollision(Xi.col(j))) {
...
template<typename ModelClass>
ClusterMPPI3D::ClusterMPPI3D(ModelClass model) : MPPI3D(model) {
    deviation_mu = 1.0;
    cost_mu = 1.0;
    minpts = 5;
    epsilon = 0.01;
...
ClusterMPPI3D::~ClusterMPPI3D() {
}

void ClusterMPPI3D::dbscan(std::vector<std::vector<int>> &clusters, const Eigen::MatrixXd &Di, const Eigen::VectorXd &costs, const int &N, const int &T) {
    clusters.clear();
...
void ClusterMPPI3D::calculateU(Eigen::MatrixXd &U, const std::vector<std::vector<int>> &clusters, const Eigen::VectorXd &costs, const Eigen::MatrixXd &Ui, const int &T) {
