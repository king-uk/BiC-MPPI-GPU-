...
#pragma omp parallel for
  ...
  elapsed_rollout += std::chrono::duration<double>(_t_bc - _t_br).count();
...
void BiMPPI::forwardRollout() {
  ...
  bool all_feasible = true;
  auto _t_fr = std::chrono::high_resolution_clock::now();
#pragma omp parallel for
  for (int i = 0; i < Nf; ++i) {
  ...
  elapsed_rollout += std::chrono::duration<double>(_t_fc - _t_fr).count();
