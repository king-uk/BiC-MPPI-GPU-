...
int main() {
  ...
  param.x_init << 2.5, 0.0, M_PI_2, 0;

  param.x_target.resize(model.dim_x);
  param.x_target << 1.5, 5.0, M_PI_2, 0;
  ...
  std::ofstream csv("result_bicycle_bi_mppi_lie.csv");
  csv << "s,map,is_failed,is_success,iter,elapsed,f_err\n";
  ...
  for (int s = 0; s < 2; ++s) {
    ...
    for (int map = 299; map >= 0; --map) {
      ...
      std::cout << s << '\t' << map << '\t' << is_failed << '\t' << is_success
                << '\t' << i << '\t' << total_elapsed << '\t' << f_err
                << std::endl;
      csv << s << ',' << map << ',' << is_failed << ',' << is_success << ','
          << i << ',' << total_elapsed << ',' << f_err << '\n';
    }
  }
